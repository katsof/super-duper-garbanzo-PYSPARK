...
newStopWords = ['This','I','The','It','would','cha','de','pas']
...
Bakery = df.filter(df.categories.contains("Bakeries"))#.show()
rdd= Bakery.rdd
bakery = rdd.filter(lambda x: x[3]== "Positive review")#.collect()

words = bakery.flatMap(lambda x:ProcessText(x[1]))
...
bakery1 = rdd.filter(lambda x: x[3]== "Negative review")#.collect()

words = bakery1.flatMap(lambda x:ProcessText(x[1]))
...
thai1 ='Thai Positive:', sorted(reviews1.collect(), key = lambda x:x[1],reverse = True)[:10]


thai2= 'Thai Negative:', sorted(reviews2.collect(), key = lambda x:x[1],reverse = True)[:10]


mex1 = 'Mexican Positive:', sorted(reviews3.collect(), key = lambda x:x[1],reverse = True)[:10]

mex2 = 'Mexican Negative:', sorted(reviews4.collect(), key = lambda x:x[1],reverse = True)[:10]



bakeries1 = 'Bakery Positive:', sorted(reviews5.collect(), key = lambda x:x[1],reverse = True)[:10]

bakeries2 = 'Bakery Negative:', sorted(reviews6.collect(), key = lambda x:x[1],reverse = True)[:10]

chinese1 = 'Chinese Positive:', sorted(reviews7.collect(), key = lambda x:x[1],reverse = True)[:10]


chinese2 = 'Chinese Negative:', sorted(reviews8.collect(), key = lambda x:x[1],reverse = True)[:10]


american1 = 'American Positive:', sorted(reviews9.collect(), key = lambda x:x[1],reverse = True)[:10]


american2 = 'American Negative:', sorted(reviews10.collect(), key = lambda x:x[1],reverse = True)[:10]

pizza1 = 'Pizza Postive:', sorted(reviews11.collect(), key = lambda x:x[1],reverse = True)[:10]
...
#bake_list_pos = sc.parallelize(list(bakery1))
#bake_list_neg = sc.parallelize(list(bakery2))
...
#final = sc.union([thai_list_pos, thai_list_neg, mex_list_pos,mex_list_neg,bake_list_pos,bake_list_neg,chinese_list_pos,chinese_list_neg,american_list_pos,american_list_neg,pizza_list_pos,pizza_list_neg])
